...
%% Grayscale
figure(02)
topleft_x=80; % number of pixels to crop from the left
topleft_y=20; % number of pixels to crop from the top
width=750; % width of the cropped frame (in pixels)
height=700; % height of the cropped frame (in pixels)
...
%% Threshold Image
figure(03)
threshold=100; % threshold value to turn grayscale into binary (0 to 255)
...
rot_min = 24; % rotating point minimum pixel radius
rot_max = 200; % rotating point maximum pixel radius
ctr_min = 10; % center point minimum pixel radius
ctr_max = 20; % center point maximum pixel radius
rot_sens = 0.97; % algorithm sensitivity to find rotating point
ctr_sens = 0.94; % algorithm sensitivity to find center point
[rotatingpoint.center,rotatingpoint.radii] = imfindcircles(test_fig_binary,[rot_min
rot_max],'ObjectPolarity','dark',...
'Sensitivity',rot_sens);
% find dark circles in the range of 10 to 30 pixel radii (in this case, this will be the
% “rotating point”)
% sensitivity denotes threshold for detection of a circle; a lower value will detect fewer
% circles
% https://urldefense.com/v3/__https://www.mathworks.com/help/images/detect-and-measure-circular-objects-in-an-image.html__;!!Mih3wA!Fq1ELDceX3UvNMfaB_siEsy5pkpnDuSUNFn1V8k1P89Y-3dv2Q8Ocsq18_P_77wstPZ70up1POvylCqC73Z6$ 
% function returns a vector of the center coordinates of each circle, and a vector of each
% circle’s radius
viscircles(rotatingpoint.center,rotatingpoint.radii);
% plot the rotating point (overlay on binary plot)
[centerpoint.center,centerpoint.radii] = imfindcircles(test_fig_binary,[ctr_min
ctr_max],'ObjectPolarity','dark',...
...
hold on
[rotatingpoint.center,rotatingpoint.radii] = imfindcircles(test_fig_binary,[rot_min
rot_max],'ObjectPolarity','dark', 'Sensitivity',rot_sens);
